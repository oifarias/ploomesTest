...
export class NewValue extends Component {
  constructor(props){
    super(props)
    ...
    this.complettid=this.complettid.bind(this)
  ...
  complettid(event) {
    this.setState({Pachid: event.target.value});
  }
  ...
  newtaskpost(){
  fetch(`https://api2.ploomes.com/Deals(${this.state.Pachid})`, {
  method: 'PATCH',
  headers: {
...
<form className="form">
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="TaskID" placeholder="Id da Tarefa" onChange={this.complettid} value={this.state.Pachid}/>
  </div>
  <div class="form-group">
  ...
  <button class="btn btn-primary btn" type="button" onClick={this.newtaskpost.bind(this)}>Atualizar </button>
</form>
  </div>  
    );
  }
